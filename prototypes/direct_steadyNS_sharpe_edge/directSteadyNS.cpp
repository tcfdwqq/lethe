       ...
       // std::cout << "count small: " << count_small<< std::endl;
        ...
        if (count_small!=0 and count_small!=local_dof_indices.size()){
            ...
            for (unsigned int k = 0; k< 2; ++k) {
                ...
                while (l < local_dof_indices.size()) {
                    Tensor<1, 2, double> vect_dist = (support_points[local_dof_indices[l]] - radius*(support_points[local_dof_indices[l]]-center_immersed)/(support_points[local_dof_indices[l]]-center_immersed).norm());
                    ...
                    if(k==0)
                    {
                            system_rhs(global_index_overrigth) =  1*((support_points[local_dof_indices[l]]-center_immersed)/(support_points[local_dof_indices[l]]-center_immersed).norm())[1]/ (dist* dist);
                    }
                    else{
                            system_rhs(global_index_overrigth) =  -1*((support_points[local_dof_indices[l]]-center_immersed)/(support_points[local_dof_indices[l]]-center_immersed).norm())[0]/ (dist* dist);
                    }
        ...
        if (count_large!=0 and count_large!=local_dof_indices.size()){
            ...
            for (unsigned int k = 0; k< 2; ++k) {
                unsigned int l = k;
                while (l < local_dof_indices.size()) {
                    Tensor<1, 2, double> vect_dist = (support_points[local_dof_indices[l]] - radius_2*(support_points[local_dof_indices[l]]-center_immersed)/(support_points[local_dof_indices[l]]-center_immersed).norm());
                    double dist = sqrt(vect_dist[1] * vect_dist[1] + vect_dist[0] * vect_dist[0]);
                    const Point<dim> second_point(support_points[local_dof_indices[l]]+vect_dist);
                    const auto &cell_2 = GridTools::find_active_cell_around_point(dof_handler, second_point);
                    Point<dim> second_point_v = immersed_map.transform_real_to_unit_cell(cell_2, second_point);
        ...
        system_matrix.set(dof_handler.n_dofs()-1,dof_handler.n_dofs()-1,1) ;
